...
The result is small in absolute terms (one to two percentage points of skill at \(K{=}1\)) but consistent across every \(K\), and we think two factors explain it.

First, the extraction approach pushes the hard semantic work onto a much larger model. The Jina embedding has 137M parameters trained on generic sentence pairs; the LLM doing the extraction is a frontier-scale model fine-tuned for instruction following. Asking the LLM to map free-form forecaster prose to a small closed vocabulary of event types is a task it does well, and the resulting token set is a much cleaner representation of ``what the AFD describes'' than a 768-dimensional dense vector trained on different objectives.

Second, the extraction representation discards exactly the kind of information that hurts the dense embeddings on this task. AFDs contain many sentences about model disagreement, forecaster reasoning, ensemble agreement, and other meta-content that doesn't predict actual weather outcomes. The dense embedding sees and partially encodes all of it; the frame extraction throws it away and keeps only the event-level structure. For a retrieval task whose ground truth is the actual weather that occurred, throwing away meta-content is the right move.

Both observations have a practical implication: on small specialized retrieval problems, an explicit pipeline that exploits a strong LLM as a feature extractor can outperform fine-tuning a smaller encoder, and the gap is wider when the relevant information is a small subset of the source text.
...
In summary: we consider the project a partial success. The negative result on the main hypothesis is itself a useful finding rather than a failure of the evaluation; we know quantitatively that contrastive fine-tuning on a corpus this size damages retrieval rather than helping it, and we have stated three concrete reasons we believe explain that result. The most useful positive finding for anyone building an AFD analog system is that a pretrained sentence transformer used zero-shot already lifts retrieval similarity meaningfully above chance, and that explicit LLM-based extraction can lift it further without any task-specific training. The least useful is the contrastive fine-tune as we implemented it: more data, refreshed mining, and a different loss function would all be needed before this approach is viable.
